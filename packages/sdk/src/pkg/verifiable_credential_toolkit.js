 ...
 * @returns {any}
 */
module.exports.normalize_object = function(input) {
    const ret = wasm.normalize_object(input);
    if (ret[2]) {
 ...
 * @returns {string}
 */
module.exports.normalize_and_stringify = function(input) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.normalize_and_stringify(input);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
...
/**
 * @param {any} unsigned_vc
 * @param {Uint8Array} private_key
 * @returns {any}
 */
module.exports.sign = function(unsigned_vc, private_key) {
    const ptr0 = passArray8ToWasm0(private_key, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.sign(unsigned_vc, ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
class KeyPair {
     ...
     * @returns {KeyPair}
     */
    static new(signing_key, verifying_key) {
     ...
     */
    signing_key() {
        const ret = wasm.keypair_signing_key(this.__wbg_ptr);
     ...
     */
    verifying_key() {
        const ret = wasm.keypair_verifying_key(this.__wbg_ptr);
        var v1 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
        wasm.__wbindgen_free(ret[0], ret[1] * 1, 1);
        return v1;
    }
...
const path = require('path').join(__dirname, 'verifiable_credential_toolkit_bg.wasm');
