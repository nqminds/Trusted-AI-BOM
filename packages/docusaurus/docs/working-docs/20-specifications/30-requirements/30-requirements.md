...
It is also worth bearing in mind that some or all of a model's training data might have been generated by other software, including by software that implements another AI model. In the latter case, TAIBOM becomes recursive, where the TAIBOM of a trained model might refer to the TAIBOMs of one or more models that were used in its production.
